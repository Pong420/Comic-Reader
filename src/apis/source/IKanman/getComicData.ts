import { api } from './api';
import { GetComicDataParam, Chapters, ComicData } from '../../../typings';
import LZString from './utils/LZString';

const chineseConv = require('chinese-conv'); // tslint:disable-line

export async function getComicData({ comicID }: GetComicDataParam): Promise<ComicData> {
  try {
    ...
    if (adultOnly || $('.warning-bar')) {
      try {
        $('#erroraudit_show').replaceWith($(LZString.decompressFromBase64($('#__VIEWSTATE').attr('value'))));
      } catch (err) {
      ...
      .each((index: number, child: CheerioElement) => {
        if ($(child).get(0).tagName === 'h4') {
          ...
          chapters[chapterType] = arr.map(chapter => {
            ...
            return {
              chapterID: $aTag.attr('href').replace(/.*\/(?=[^\/].*$)|.html/g, ''),
